...
def main():
	...
	parser.add_argument("--style_file", default=None, type=str,
						help="The output style response/know data file name.")
	#Dev file
	...
	parser.add_argument("--dev_tgt_file", default=None, type=str,
						help="The output data file name.")
	parser.add_argument("--dev_check_file", default=None, type=str,
						help="The output style response/know data file name.")
	parser.add_argument("--dev_style_file", default=None, type=str,
						help="The output style response/know data file name.")
	#KS File
	...
	parser.add_argument("--train_vae",action='store_true',help="Whether to train vae.")
	...
	if args.do_train:
		...
		fn_style = os.path.join(args.data_dir, args.style_file)
		train_dataset = seq2seq_loader.C_Seq2SeqDataset(fn_src, fn_tgt, fn_check, fn_style, args.train_batch_size, data_tokenizer, args.max_seq_length,
		...
		fn_style = os.path.join(args.data_dir, args.dev_style_file)
		fn_check = os.path.join(args.data_dir, args.dev_check_file)
		dev_reddit_dataset = seq2seq_loader.C_Seq2SeqDataset(fn_src, fn_tgt, fn_check, fn_style, args.eval_batch_size, data_tokenizer, args.max_seq_length,
															 file_oracle=file_oracle, bi_uni_pipeline=C_bi_uni_pipeline)
		if args.local_rank == -1:
		...
		dev_reddit_dataloader = torch.utils.data.DataLoader(dev_reddit_dataset, batch_size=_batch_size, sampler=dev_reddit_sampler, num_workers=args.num_workers,
															collate_fn=seq2seq_loader.batch_list_to_batch_tensors, pin_memory=False)
		...
		ks_dev_reddit_dataset = seq2seq_loader.Seq2SeqDataset(ks_dev_fn_src, ks_dev_fn_tgt, args.eval_batch_size, data_tokenizer, args.max_seq_length,
		...
		if args.local_rank == -1:
			...
			_batch_size = args.eval_batch_size // dist.get_world_size()
		ks_dev_reddit_dataloader = torch.utils.data.DataLoader(ks_dev_reddit_dataset, batch_size=_batch_size, sampler=ks_dev_reddit_sampler, num_workers=args.num_workers,
															   collate_fn=seq2seq_loader.batch_list_to_batch_tensors, pin_memory=False)
		...
		for i_epoch in trange(start_epoch, start_epoch + 1, desc="Epoch", disable=args.local_rank not in (-1, 0)):
			...
			for batch, ks_batch in zip(train_dataloader, ks_train_dataloader):
				...
				loss_tuple = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next, masked_pos=masked_pos,
								   ...
								   masked_weights_2=oracle_weights, masked_labels_2=oracle_labels, mask_qkv=mask_qkv, tgt_pos=tgt_pos, labels=labels,
								   ks_labels=ks_labels, train_vae=args.train_vae, style_ids=style_ids,
								   style_labels=style_labels, check_ids=check_ids, pretrain=pretrain)

				if args.train_vae:
					masked_lm_loss, next_sentence_loss, KL_loss, Mutual_loss = loss_tuple
				...
				KL_weight += (0.1 / 200000)

				if args.train_vae:
					loss = masked_lm_loss + next_sentence_loss + Mutual_loss + KL_weight * KL_loss
				else:
					if step <= pretrain_step:
						loss = Golden_loss
				...
				if random.randint(0, 4) == 0 and step > pretrain_step:
					...
					loss_tuple = model(input_ids, segment_ids, input_mask, lm_label_ids, is_next, masked_pos=masked_pos,
									   ...
									   ks_labels=ks_labels, train_ks=True, train_vae=args.train_vae)
					if args.train_vae:
				...
				if (global_step + 1) % 5000 == 0:
					...
					data_path = os.path.join(args.data_dir, "test_qkr_dev.ks_score.tk")
					src_path = os.path.join(args.data_dir, "test_qkr_dev.src.tk")
					src_out_path = os.path.join(args.data_dir, "rank_top5_test_qkr_dev.src.tk")
					tgt_path = os.path.join(args.data_dir, "test_qkr_dev.tgt")

					knowledge_selection(data_path, src_path, src_out_path)
					logger.info("************ DEV Know Selection End ************")

					# Decode Stage
					logger.info("************ Dev Decod Begin ************")
					with open(src_out_path, encoding="utf-8") as file:
						dev_src_lines = file.readlines()
					with open(tgt_path) as file:
						golden_response_lines = file.readlines()

					out = codecs.open("{}_decode.txt".format(step), "w")
					decode_result = decode_batch(model, dev_src_lines)
					for decode_line in decode_result:
						out.write(decode_line + "\n")
					logger.info("************ Dev Decod End ************")
