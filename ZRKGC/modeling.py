...
from pytorch_bert.file_utils import cached_path
from pytorch_bert.loss import LabelSmoothingLoss
import nltk
...
class BertEmbeddings(nn.Module):
    ...
    def get_full_embedding(self, input_ids, token_type_ids=None, position_ids=None, task_idx=None,relace_embeddings=None,latent_z=None):
        seq_length = input_ids.size(1)
        if position_ids is None:
            position_ids = torch.arange(
                seq_length, dtype=torch.long, device=input_ids.device)
...
class BertModel(PreTrainedBertModel):
    ...
    def get_full_embedding(self,input_ids,token_type_ids,task_idx=None,relace_embeddings=None,latent_z=None):
